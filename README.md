...
Due to the stochastic nature of the dorado particle tracking model, exact reproduction of manuscript figures requires using the archived dorado_results available on Zenodo.
...
This project requires a modified version of `dorado` that will soon be merged with the main parent repo at passah20/dorado. Until the PR is merged into the main repo, install from this fork (tested on commit fa64f59):
Example workflow:
...
1. processing_scripts/dorado_add_ROI.ipynb - this is optional.
You can alternatively download the dorado results from Zenodo instead of re-running this step.
...
## Versioning
Version: v1.0 (corresponding to manuscript submission)
